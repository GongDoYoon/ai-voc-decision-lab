...
- LLM 실패 시에도 동작하는 로컬 규칙 엔진과 선택적 OpenAI-compatible 어댑터
...
별도 API 키 없이 데모가 동작합니다. 실제 LLM 비교 실험은 `.env.example`의 세 값을 설정하면 `/api/analyze`가 선택적으로 사용합니다. 오류·타임아웃·잘못된 인용이 발생하면 안전하게 로컬 결과로 되돌아갑니다.
...
- 다음 단계는 피드백 업로드, 검토 큐, 프롬프트 버전별 비용·지연시간 관측입니다.
